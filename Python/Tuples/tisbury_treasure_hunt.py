...
def get_coordinate(record):
    """Return coordinate value from a tuple containing the treasure name, and
    treasure coordinate.
...
def create_record(a_record, r_record):
    ...
    (if incompatible).
    """
    return (*a_record, *r_record) if compare_records(a_record, r_record) else 'not a match'
...
def clean_up(combined_record_group):
    ...
    """
    return ''.join([f"('{_[0]}', '{_[2]}', {_[3]}, '{_[4]}')\n" for _ in combined_record_group])
