...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    A reactor is said to be critical if it satisfies the following
    conditions:
...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    :param theoretical_max_power: int or float - power that corresponds to
    a 100% efficiency.
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    `threshold`
    3. 'DANGER' -> `temperature * neutrons per second` is not in the above-
    stated ranges
    """
